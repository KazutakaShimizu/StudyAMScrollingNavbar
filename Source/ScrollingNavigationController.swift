...
@objc public protocol ScrollingNavigationControllerDelegate: NSObjectProtocol {
   ...
   */
//  protocolだが、optionalがついてるので実装が必須ではないメソッド
//  ステータスバーのstateが変わる直前に呼ばれるメソッド
...
//  初期値はexpanded
  open fileprivate(set) var state: NavigationBarState = .expanded {
    willSet {
//      新しい値が入ってきたら、willChangeStateを呼ぶ
      if state != newValue {
        scrollingNavbarDelegate?.scrollingNavigationController?(self, willChangeState: newValue)
      }
    }
    didSet {
//      現在のstateがexpandedならisUserInteractionEnabledにはtrueが、そうでないならfalseが入る
//      expanded以外の状況の時は、ユーザーの操作を無効にする
//      これがないと、navigationBarが出てない時でも、ナビゲーションアイテムがある位置をタップするとそのアクションが実行されてしまう
